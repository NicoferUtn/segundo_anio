...
--pasarPorEventos eventos ciudad = foldl aplicarEvento ciudad eventos
...
aBajarCostoDeVida unAño = transformarUnaCiudad unAño paraBajarCostoDeVida 
...
paraBajarCostoDeVida :: Ciudad -> Evento -> Bool
paraBajarCostoDeVida unaCiudad unEvento = not (terminaMejorSegun costoDeVida unaCiudad unEvento)
--------------------------Funciones para no repetir lógica----------------------------------------
...
algoEstaOrdenado modificadorDeElementos elementosAOrdenar = esCreciente (map (costoDeVida . modificadorDeElementos) elementosAOrdenar)
...
--Punto 7
año2024 :: Año
año2024 = Año 2024 (ciudadEnCrisis : ciudadARemodelar 7  : eventosInfinitos)
...
{- 
Dado que la funcion del punto 6.3 está definida como
si el costo de vida tras pasar un año es menor al costo de vida tras pasar el año siguiente,
sin utilizar menor o igual, al llegar al punto donde se repite el año
2023, el costo de vida de una ciudad al pasar por 2023, es igual al de 
la misma ciudad al pasar por el mismo año, por lo que se obtendrá un resultado
False, y, dado que False es el elemento absorbente del AND y haskell cuenta
con Lazy evaluation, no evaluara el resto de la lista, sino que devolverá
False
-}